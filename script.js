...
async function handleUpdateBike() {
    ...
    if (file) {
        ...
        try {
            ...
            // Si el usuario subió una imagen nueva, esta se convierte en la nueva URL
...
async function handleUpdateMaintenance() {
    ...
    const file = fileInput.files[0];
    const deleteCheckbox = document.getElementById('delete-current-maintenance-image');

    let imageURL = maintenance.imageURL; // Mantener la URL antigua por defecto

    // 1. PROCESAR BORRADO (Si la casilla está marcada)
    if (deleteCheckbox && deleteCheckbox.checked && maintenance.imageURL) {
        saveBtn.textContent = 'Eliminando imagen antigua...';
        const deleted = await deleteImageFromStorage(maintenance.imageURL);
        if (deleted) {
            imageURL = null; // Borramos la URL de Firestore
        } else {
            saveBtn.disabled = false;
    ...
    const updatedData = {
        ...
        next_maintenance_km: nextMaintenanceKm,
        imageURL: imageURL, // Actualización de imagen
        id: maintenanceId
    };
...
function showEditMaintenanceModal(maintenanceId) {
  ...
  const imageInfo = document.getElementById('edit-maintenance-image-info');
  if (imageInfo) {
    if (maintenance.imageURL) {
        imageInfo.innerHTML = `<p style="margin-top: 10px;">✅ Imagen actual adjunta. <label><input type="checkbox" id="delete-current-maintenance-image"> Eliminar imagen actual</label></p>`;
    } else {
        imageInfo.innerHTML = `<p style="margin-top: 10px;">❌ No hay imagen adjunta.</p>`;
    }
...
// ARREGLO DEFINITIVO DE RENDERSTOCK
function renderStock() {
  ...
  stockList.innerHTML = stock.map(s => {
    ...
    // Lógica de imagen: Si hay URL usa <img> (pequeña), si no, un icono
    let imageHTML = '';
    if (s.imageURL) {
